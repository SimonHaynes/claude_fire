...
| Never-uprated allowances (Lump Sum Allowance, ISA limit, CGT exempt amount, dividend allowance) | **Frozen in cash terms, forever** — erode at 2%/yr real. `FiscalDrag.allowance_inflation`, on by default | No indexation provision in the legislation | Taking tax-free cash earlier than an indexed allowance would justify |
| Income tax, NI and IHT thresholds over time | Frozen to **April 2031**, then uprated, so they stop losing real value. `FiscalDrag.inflation`, **off by default** — set it per household | Budget, 26 Nov 2025; `FiscalDrag.verified_on` | Understates drag while off, and if the freeze is extended again as it has been twice |
